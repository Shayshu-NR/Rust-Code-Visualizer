...
def extract_config_info(proj_dir):
    ...
    if not os.path.isdir(proj_dir):
        raise RuntimeError(
            f"Given project directory is not a directory: {proj_dir}")

    toml_path = os.path.join(proj_dir, "Cargo.toml")
    if not os.path.isfile(toml_path):
        raise RuntimeError(
            f"Given project directory does not contain a 'Cargo.toml' file: {proj_dir}")
    ...
    if bin_name is None:
        raise RuntimeError(
            "Given Cargo.toml file in project directory does not specify a bin name")
...
def convert_to_json(file_class, bin_name, output_path):
    ...
    """
    dot_file = os.path.join(file_class.proj_dir,
                            OUTDIR_NAME, DOT_INTERMEDIATE_NAME)
...
def generate_call_graph(args):
    ...
    if not args.output_path:
        output_path = os.path.join(
            file_class.proj_dir, OUTDIR_NAME, CYTO_OUTPUT_NAME)
    else:
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Script to generate the call graph of a Rust source code file.")
    parser.add_argument("-p", "--proj_dir", default=".",
                        help="Directory containing Cargo.toml file and rust source files")
    #parser.add_argument("-f","--rust_file", required=True, help="Rust source code file.")
    parser.add_argument("-c", "--compiler", default="x86_64-unknown-linux-gnu",
                        help="Compiler target to use when generating a call stack")
    parser.add_argument("-o", "--output_path",
                        help="Path to store output cytoscape JSON file")
