...
class profiler:
    ...
    def get_profiling_info(self, exec):
        try:
            self.__executable = exec.split("/")[-1]
    ...
    def __run_profilers(self):
        profiler_cmd = "valgrind --tool=callgrind --branch-sim=yes --cache-sim=yes ./" + \
            self.__executable + " >/dev/null 2>&1"
        os.system(profiler_cmd)
        callgrind_out = None
        for file in os.listdir(self.__current_path):
        ...
        if callgrind_out == None:
            raise Exception(
                "Please make sure valgrind is installed before proceeding!")

        output_file_cmd = "callgrind_annotate  --tree=both --inclusive=yes " + \
            callgrind_out + " > " + self.__parse_file
    ...
    def __extract_data(self):
        ...
        FUNCTION_DATA_ALL = re.compile(r".*:" + re.escape(os.path.split(self.__current_path + "/" + self.__executable)[
                                       1]) + r".*\[" + re.escape(self.__current_path + "/" + self.__executable) + r"\].*")  # Inclusive of caller/callees
            ...
            if(search != None):
                ...
                while not lines[i].isspace():
                    ...
                    if(search != None):
                        ...
                        for j in range(0, 13):
                            data_list[j] = str(
                                int(data_list[j]) - int(callee_line[j]))
                    i += 1
    ...
    def __create_json(self):
        graph_data = ["L1 Data Cache Misses", "LL Data Cache Misses",
                      "Instruction Count", "Branch Mispredictions"]
        graph_colors = ["#36a2eb",
                        "#ff6384",
                        "#4bc0c0",
                        "#ff9f40"]
        ...
        for dataset in graph_data:
            ...
            if dataset == "L1 Data Cache Misses":
                for func in func_labels:
                    graph_data_list.append(int(self.__func_dict[func]["First level data cache read misses"]) + int(
                        self.__func_dict[func]["First level data cache write misses"]))
            elif dataset == "LL Data Cache Misses":
                for func in func_labels:
                    graph_data_list.append(int(self.__func_dict[func]["Last level data cache read misses"]) + int(
                        self.__func_dict[func]["Last level data cache write misses"]))
            else:
                for func in func_labels:
                    graph_data_list.append(
                        int(self.__func_dict[func][dataset]))
    ...
    def __print_info(self):
        for key in self.__func_dict.keys():
            ...
            for data_key in self.__func_dict[key].keys():
                print(data_key + ": " + self.__func_dict[key][data_key] + " (" + str(int(int(
                    self.__func_dict[key][data_key])*100/int(self.__func_dict["Totals"][data_key]))) + "%)")
